...
?>
	<div class="postbox cookiebot-addon">
		<p>
			<label for="<?php echo 'enabled_' . $addon::OPTION_NAME; ?>"><?php esc_html_e( 'Enable', 'cookie' ); ?></label>
			<input type="checkbox" id="<?php echo 'enabled_' . $addon::OPTION_NAME; ?>"
				   name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][enabled]"
		...
		<ul class="cookietypes">
			<li><input type="checkbox" id="cookie_type_preferences_<?php echo $addon::OPTION_NAME; ?>"
					   value="preferences"
					<?php cookiebot_addons_checked_selected_helper( $addon->get_cookie_types(), 'preferences' ); ?>
					   name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][cookie_type][]"><label>Preferences</label>
			</li>
			<li><input type="checkbox" id="cookie_type_statistics_<?php echo $addon::OPTION_NAME; ?>"
					   value="statistics"
					<?php cookiebot_addons_checked_selected_helper( $addon->get_cookie_types(), 'statistics' ); ?>
					   name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][cookie_type][]"><label>Statistics</label>
			</li>
			<li><input type="checkbox" id="cookie_type_marketing_<?php echo $addon::OPTION_NAME; ?>"
					   value="marketing"
					<?php cookiebot_addons_checked_selected_helper( $addon->get_cookie_types(), 'marketing' ); ?>
					   name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][cookie_type][]"><label>Marketing</label>
			</li>
		</ul>
		...
		<p>
			...
			<input type="checkbox"
				   ...
				   data-addon="<?php echo $addon::OPTION_NAME; ?>"
				   name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][placeholder][enabled]"
				<?php checked( 1, $addon->is_placeholder_enabled() ); ?>
				   value="1">
		</p>

		<div class="placeholder"
			 data-addon="<?php echo $addon::OPTION_NAME; ?>" <?php echo ( ! $addon->is_placeholder_enabled() ) ? 'style="display:none"' : ''; ?>>
				...
				<?php foreach ( $addon->get_placeholders() as $placeholder_lang => $placeholder_value ) : ?>
					<div class="placeholder_content submitbox">
						<p>
							...
							<?php
							$name = 'cookiebot_available_addons[' . $addon::OPTION_NAME . '][placeholder][languages][' . $placeholder_lang . ']';
						...
						<textarea cols="60" rows="5"
								  name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][placeholder][languages][<?php echo $placeholder_lang; ?>]"><?php echo $placeholder_value; ?></textarea>
				...
				<div class="placeholder_content">
					...
					<p>
						<textarea cols="80" rows="5"
								  name="cookiebot_available_addons[<?php echo $addon::OPTION_NAME; ?>][placeholder][languages][site-default]"><?php echo $addon::DEFAULT_PLACEHOLDER_CONTENT; ?></textarea>
						<span class="help-tip" title="<?php echo $addon->get_placeholder_helper(); ?>"></span>
					</p>
				</div>
			<?php endif; ?>
			...
			<p class="add_placeholder_language">
				<button class="btn_add_language button button-secondary"
						data-addon="<?php echo $addon::OPTION_NAME; ?>"><?php esc_html_e( '+ Add language', 'cookiebot-addons' ); ?></button>
			</p>
		</div>
		<?php
