 ...
 * @since 1.2.0
 */
function cookiebot_addons_remove_class_action( $action, $class_name, $method, $priority = 10 ) {
	global $wp_filter;
	$deleted = false;
 ...
 * @since 1.8.0
 */
function cookiebot_addons_get_dropdown_languages( $class_name, $name, $selected ) {
	$args     = array(
		'name'                     => $name,
		'selected'                 => $selected,
	...
	$output = str_replace( 'select ', 'select class="' . $class_name . '" ', $dropdown );
