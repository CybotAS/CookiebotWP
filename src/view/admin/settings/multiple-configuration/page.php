...
<div class="cb-multiple__container <?= !get_option( 'cookiebot-multiple-config' ) ? 'hidden' : '' ; ?>">
    <div class="cb-settings__config__item">
        <div class="cb-settings__config__content">
    ...
    <div class="cb-region__table">
        ...
        <div class="cb-region__table__item">
            <div class="cb-region__item__group">
                <input type="text" disabled placeholder="<?= $cbid ? $cbid : '1111-1111-1111-1111'; ?>">
            </div>
        ...
        <div class="cb-region__table__item">
            <div class="cb-region__item__group">
                <input type="text" name="cookiebot-second-banner-id" placeholder="1111-1111-1111-1111" value="<?= $secondary_group_id; ?>">
            </div>
            <div class="cb-region__item__region">
                <input type="hidden" name="cookiebot-second-banner-regions" id="second-banner-regions" value="<?= implode(', ', array_keys($selected_regions)); ?>">
                <input type="hidden" name="cookiebot-ccpa" id="ccpa-compatibility" value="<?= $ccpa_compatibility; ?>">
                <div class="cb-region__region__selector">
                    <div class="default-none <?= $selected_regions ? 'hidden' : '' ; ?>"><?php esc_html_e( 'Select region', 'cookiebot' ); ?></div>
                    <div class="selected-regions">
                        <?php foreach($selected_regions as $code => $region) : ?>
                            <div id="<?= $code; ?>" class="selected-regions-item"><?= $region; ?></div>
                        <?php endforeach; ?>
                    </div>
                </div>
                <div class="cb-region__region__list hidden">
                    ...
                    <?php foreach ($supported_regions as $code => $region) : ?>
                        <div class='cb-region__region__item <?= array_key_exists($code,$selected_regions) ? 'selected-region' : ''; ?>' data-region="<?= $code; ?>"><?= $region; ?></div>
                    <?php endforeach; ?>
                    </div>
                </div>
            </div>
