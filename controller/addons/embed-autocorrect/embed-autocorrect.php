...
class Embed_Autocorrect {
	...
	public function cookiebot_addon_embed_autocorrect() {
		...
		add_filter('the_content', array($this,'cookiebot_addon_embed_autocorrect_content'),1000); //Ensure it is executed as the last filter
		
		//add filters to handle autocorrection in widget text
		add_filter('widget_text', array($this,'cookiebot_addon_embed_autocorrect_content'),1000); //Ensure it is executed as the last filter
	}
